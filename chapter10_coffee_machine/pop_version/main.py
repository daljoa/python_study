...
# 이상의 함수들의 조건을 다 통과했다면 -> 음료 제조
def make_coffee(drink_name, order_ingredients):
    """resources에 있는 재료에서 MENU["음료이름"]["재료"]들을 차감함. -> 차감은 무조건 이루어지죠
    -> is_resource_enough을 통과했기 때문에"""
    # 어떻게하면 resources의 재료들에서 해당 음료의 필요 재료들을 차감할 수 있을지를 고민 -> 어제 했다
    # order_ingredients만큼 차감할 수 있을지 고민해보고 작성하세요.
    # 힌트 : for 반복문 -> MENU["음료이름"]["재료"]의 value가 dictionary.
    for item in order_ingredients:
